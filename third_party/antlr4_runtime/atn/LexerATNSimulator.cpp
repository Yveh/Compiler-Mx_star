...
size_t LexerATNSimulator::execATN(CharStream *input, dfa::DFAState *ds0) {
  ...
  while (true) { // while more work
    // As we move antlr4_runtime->trg, antlr4_runtime->trg, we keep track of the previous trg to
    ...
    t = input->LA(1);
    s = target; // flip; current DFA target becomes new antlr4_runtime/from state
  }
